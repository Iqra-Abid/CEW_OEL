...
void send_notification(const char* title, const char* message) {
    ...
    // Format the Zenity command (without background execution)
...
void sanitize_string(char* str) {
    for (int i = 0; str[i] != '\0'; i++) {
        // Replace non-printable characters (below 32 or above 126) with a space
...
void calculate_and_display_averages(cJSON* weather_json, int days_count) {
    ...
    for (int i = 0; i < days_count; i++) {
        ...
        double wind_speed = cJSON_GetObjectItem(day, "maxwind_kph") ? cJSON_GetObjectItem(day, "maxwind_kph")->valuedouble : 0; // Assuming wind speed is in kilometers per hour
        ...
        total_wind_speed += wind_speed; // Sum the wind speed
    ...
    // Ensure days_count is not zero to avoid division by zero
    if (days_count == 0) {
    ...
    double avg_wind_speed = total_wind_speed / days_count; // Calculate average wind speed
